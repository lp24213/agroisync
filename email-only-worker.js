  ...
  async fetch(request, env) {
    ...
    if (url.pathname === "/api/create-admin" && request.method === "POST") {
      try {
        ...
        console.log(
          `✅ ADMIN CRIADO: ${adminEmail} (ID: ${result.meta.last_row_id})`,
        );
    ...
    if (
      url.pathname === "/api/email/send-verification" &&
      request.method === "POST"
    ) {
      try {
        ...
        try {
          ...
          console.log(
            "Erro ao salvar no banco (continuando):",
            dbError.message,
          );
        }

        console.log(
          `🚀 ENVIANDO EMAIL via RESEND para ${email} com código ${verificationCode}`,
        );
        ...
        try {
          ...
          const emailData = {
            from: env.RESEND_FROM || "Agroisync <contato@agroisync.com>",
            to: email,
            subject: "Seu código de verificação - Agroisync",
            html: `
              <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
                <div style="text-align: center; margin-bottom: 30px;">
                  <h1 style="color: #059669; margin: 0;">🌾 Agroisync</h1>
                ...
                <div style="margin-top: 30px; padding-top: 20px; border-top: 1px solid #eee;">
                  <p style="color: #999; font-size: 14px; margin: 0;">
                    ...
                    Agroisync - Conectando o agronegócio brasileiro.
                  </p>
                </div>
              </div>
            `,
          ...
          if (resendResponse.ok && resendData.id) {
            console.log(
              `📧 EMAIL ENTREGUE via Resend para ${email}: ${verificationCode}`,
            );
      ...
      try {
        ...
        try {
          ...
          const resendResponse = await fetch("https://api.resend.com/emails", {
            ...
            body: JSON.stringify({
              from: "Agroisync <contato@agroisync.com>",
              to: email,
          ...
          if (resendResponse.ok && resendData.id) {
            console.log(
              `📧 EMAIL DE RECUPERAÇÃO ENTREGUE via Resend para ${email}: ${resetCode}`,
            );
            ...
            return new Response(
              JSON.stringify({
                success: true,
                message:
                  "Email de recuperação ENTREGUE! Verifique sua caixa de entrada.",
                data: {
                  email,
                  resetCode,
                  messageId: resendData.id,
    ...
    if (url.pathname === "/api/auth/register" && request.method === "POST") {
      try {
        const { email, password, name, phone, turnstileToken } =
          await request.json();
          ...
          const turnstileResponse = await fetch(
            "https://challenges.cloudflare.com/turnstile/v0/siteverify",
            {
              ...
              body: new URLSearchParams({
                secret:
                  env.CLOUDFLARE_TURNSTILE_SECRET || "0x4AAAAAAA2rOqUOZqKxZqKx",
    ...
    if (
      url.pathname === "/api/products/register" &&
      request.method === "POST"
    ) {
      try {
        const {
        ...
        console.log(
          `✅ Produto cadastrado: ${name} (ID: ${result.meta.last_row_id})`,
        );
    ...
    if (url.pathname === "/api/freight/register" && request.method === "POST") {
      try {
        ...
        console.log(
          `✅ Frete cadastrado: ${driverName} (ID: ${result.meta.last_row_id})`,
        );
    ...
    if (url.pathname === "/api/stores/register" && request.method === "POST") {
      try {
        ...
        console.log(
          `✅ Loja cadastrada: ${storeName} (ID: ${result.meta.last_row_id})`,
        );
    ...
    if (
      url.pathname === "/api/secure-urls/generate" &&
      request.method === "POST"
    ) {
      try {
    ...
    if (
      url.pathname.startsWith("/api/secure-urls/verify/") &&
      request.method === "GET"
    ) {
      try {
    ...
    if (
      url.pathname === "/api/secure-urls/validate" &&
      request.method === "POST"
    ) {
      try {
    ...
    if (
      url.pathname === "/api/secure-urls/invite" &&
      request.method === "POST"
    ) {
      try {
        ...
        const timestamp = Date.now();
        const inviteCode = btoa(`${referrerId}:${type}:${timestamp}`).replace(
          /=/g,
          "",
        );
    ...
    if (
      url.pathname.startsWith("/api/secure-urls/verify-invite/") &&
      request.method === "GET"
    ) {
      try {
        const inviteCode = url.pathname.split(
          "/api/secure-urls/verify-invite/",
        )[1];
        ...
        try {
          const decoded = atob(
            inviteCode + "==".slice(inviteCode.length % 4 || 4),
          );
          const parts = decoded.split(":");
          if (parts.length === 3) {
