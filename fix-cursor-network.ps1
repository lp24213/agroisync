...
# 1. Adicionar exceção no Firewall para o Cursor
Write-Host "📝 Adicionando exceção no Firewall para o Cursor..." -ForegroundColor Cyan
...
if (Test-Path $cursorPath) {
    ...
    Write-Host "✅ Exceções do Firewall adicionadas!" -ForegroundColor Green
} else {
    Write-Host "⚠️ Cursor não encontrado no caminho padrão" -ForegroundColor Yellow
}

# 2. Configurar DNS público
Write-Host "📝 Configurando DNS público..." -ForegroundColor Cyan
...
Write-Host ""
Write-Host "🎯 SOLUÇÕES ADICIONAIS:" -ForegroundColor Yellow
Write-Host "1. Reinicie o Cursor completamente" -ForegroundColor White
Write-Host "2. Se ainda não funcionar, configure proxy manualmente:" -ForegroundColor White
