...
const FarmCard = ({ farm, onStake, onUnstake, onHarvest }: FarmCardProps) => {
  ...
  const handleStake = () => {
    ...
    setTimeout(() => {
      onStake(farm.id, parseFloat(stakeAmount));
      setIsStaking(false);
      setStakeAmount('');
    }, 1500);
  };
  ...
  const handleUnstake = () => {
    if (!unstakeAmount || parseFloat(unstakeAmount) <= 0 || parseFloat(unstakeAmount) > farm.userStaked) return;
    
    setIsUnstaking(true);
    
    // Simulate unstaking process
    setTimeout(() => {
      onUnstake(farm.id, parseFloat(unstakeAmount));
              ...
              <div className="border-t border-gray-800 p-5">
                ...
                <div className="mb-6">
                  ...
                  <motion.button
                    ...
                    whileHover={stakeAmount && parseFloat(stakeAmount) > 0 && !isStaking ? { scale: 1.02 } : {}}
                    whileTap={stakeAmount && parseFloat(stakeAmount) > 0 && !isStaking ? { scale: 0.98 } : {}}
                  >
                    {isStaking ? (
                      <span className="flex items-center justify-center">
                        <RefreshCw className="h-4 w-4 mr-2 animate-spin" />
                ...
                {farm.userStaked > 0 && (
                  <div>
                    ...
                    <motion.button
                      ...
                      disabled={!unstakeAmount || parseFloat(unstakeAmount) <= 0 || parseFloat(unstakeAmount) > farm.userStaked || isUnstaking}
                      ...
                      whileHover={unstakeAmount && parseFloat(unstakeAmount) > 0 && parseFloat(unstakeAmount) <= farm.userStaked && !isUnstaking ? { scale: 1.02 } : {}}
                      whileTap={unstakeAmount && parseFloat(unstakeAmount) > 0 && parseFloat(unstakeAmount) <= farm.userStaked && !isUnstaking ? { scale: 0.98 } : {}}
                    >
                      {isUnstaking ? (
                        <span className="flex items-center justify-center">
                          <RefreshCw className="h-4 w-4 mr-2 animate-spin" />
