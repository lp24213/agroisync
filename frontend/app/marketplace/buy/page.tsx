...
const MarketplacePage: React.FC = () => {
  ...
  const [purchaseSuccess, setPurchaseSuccess] = useState<{txHash: string, itemId: string} | null>(null);
    ...
    <div className="container mx-auto py-8 px-4 max-w-7xl">
      ...
      {purchaseSuccess && (
        <div className="mb-6 p-4 bg-green-50 rounded-md border-l-4 border-green-500">
          <h3 className="text-lg font-semibold text-green-600 mb-2">Compra realizada com sucesso!</h3>
          <p>O item <strong>{mockMarketplaceItems.find(item => item.id === purchaseSuccess.itemId)?.name}</strong> foi adquirido.</p>
          <p className="mt-1">Hash da transação: <span className="font-medium text-blue-600">{purchaseSuccess.txHash}</span></p>
        </div>
      )}
            ...
            >
              <div 
                className={cn(
                  ...
                  item.type === 'token' ? 'bg-blue-500' : item.type === 'nft' ? 'bg-purple-500' : 'bg-orange-500'
                )}
              >
          ...
          <ModalBody>
            {selectedItem && (
              <BuyWithCommission
                ...
                tokenType={selectedItem.type === 'token' ? 'ERC20' : selectedItem.type === 'nft' ? 'ERC721' : 'ERC1155'}
