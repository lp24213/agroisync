...
class ChatbotService {
  ...
  async generateMockAIResponse(message) {
    ...
    if (lowerMessage.includes('cadastro') || lowerMessage.includes('registro')) {
      return {
        content:
          'Para fazer seu cadastro no AgroSync, clique em "Cadastrar" no menu superior. Você precisará fornecer: nome completo, e-mail, telefone e CPF/CNPJ. Após o cadastro, faremos a verificação dos dados com a Receita Federal.',
        suggestions: [
      ...
      return {
        content:
          'O AgroSync funciona como uma plataforma de intermediação. Quando você tem interesse em um produto ou frete, registramos sua intenção e abrimos um canal de comunicação direto com o vendedor/anunciante. A plataforma não realiza vendas diretas, apenas conecta compradores e vendedores.',
        suggestions: [
