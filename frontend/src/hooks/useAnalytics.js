...
export const useAnalytics = () => {
  ...
  const trackButtonClick = useCallback((buttonName, location, details = {}) => {
  ...
  const trackSearch = useCallback((query, results, category = null) => {
  ...
  const trackProductView = useCallback((productId, productName, category, price) => {
  ...
  const trackBeginCheckout = useCallback((items, totalValue) => {
  ...
  const trackPurchase = useCallback((transactionId, items, totalValue, paymentMethod) => {
  ...
  const trackSignUp = useCallback((method, userType) => {
  ...
  const trackTimeOnPage = useCallback((pageName, timeSpent) => {
  ...
  const trackScroll = useCallback((pageName, scrollPercentage) => {
  ...
  const trackShare = useCallback((contentType, contentId, method) => {
  ...
  const trackPageError = useCallback((errorCode, errorMessage, pageUrl) => {
  ...
  const trackApiError = useCallback((endpoint, errorCode, errorMessage) => {
