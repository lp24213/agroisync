...
const AdminCard = ({ type, data, onAction, onView, onEdit, onDelete, onBan, onUnban, onExport, onImport }) => {
  ...
  const getCardIcon = type => {
    const icons = {
      ...
      freight: <Truck className='h-6 w-6' />,
      payment: <DollarSign className='h-6 w-6' />,
      system: <Settings className='h-6 w-6' />,
      activity: <Activity className='h-6 w-6' />,
      security: <Shield className='h-6 w-6' />,
      database: <Database className='h-6 w-6' />
    }
    return icons[type] || <BarChart3 className='h-6 w-6' />
  }
        ...
        return (
          <div className='space-y-3'>
            <div className='flex items-center justify-between'>
              ...
              <span
                className={`rounded-full border px-2 py-1 text-xs font-medium ${getStatusColor(data.status)} flex items-center space-x-1`}
              >
            ...
            {data.lastLogin && <div className='text-xs text-slate-500'>Último login: {formatDate(data.lastLogin)}</div>}
          </div>
        )
      ...
      case 'product':
        return (
          <div className='space-y-3'>
            ...
            {data.description && <p className='line-clamp-2 text-sm text-slate-600'>{data.description}</p>}
          </div>
        )
      ...
      case 'freight':
        return (
          <div className='space-y-3'>
            <div className='flex items-center justify-between'>
              <div>
                ...
                <p className='text-sm text-slate-600'>
                  {data.origin || 'Origem'} → {data.destination || 'Destino'}
                </p>
              </div>
              <span
                className={`rounded-full border px-2 py-1 text-xs font-medium ${getStatusColor(data.status)} flex items-center space-x-1`}
            ...
            <div className='text-xs text-slate-500'>Criado: {formatDate(data.createdAt)}</div>
          </div>
        )
      ...
      case 'payment':
        return (
          <div className='space-y-3'>
            ...
            <div className='text-xs text-slate-500'>Data: {formatDate(data.date)}</div>
          </div>
        )
      ...
      case 'system':
        return (
          <div className='space-y-3'>
            ...
            {data.lastUpdate && (
              <div className='text-xs text-slate-500'>Última atualização: {formatDate(data.lastUpdate)}</div>
            )}
          </div>
        )
  ...
  const renderActions = () => {
    ...
    if (onView) actions.push({ label: 'Ver', icon: <Eye className='h-4 w-4' />, action: () => handleView(data.id) })
    if (onEdit)
      actions.push({ label: 'Editar', icon: <Edit className='h-4 w-4' />, action: () => handleEdit(data.id) })
    if (onDelete)
      actions.push({ label: 'Excluir', icon: <Trash className='h-4 w-4' />, action: () => handleDelete(data.id) })
    if (onBan && data.status !== 'banned')
      actions.push({ label: 'Banir', icon: <Ban className='h-4 w-4' />, action: () => handleBan(data.id) })
    if (onUnban && data.status === 'banned')
      actions.push({ label: 'Desbanir', icon: <Unlock className='h-4 w-4' />, action: () => handleUnban(data.id) })
    if (onExport)
      actions.push({ label: 'Exportar', icon: <Download className='h-4 w-4' />, action: () => handleExport(data.id) })
    ...
    return actions.map((action, index) => (
      <motion.button
        ...
        className='flex items-center space-x-2 rounded-lg px-3 py-2 text-sm text-slate-600 transition-colors duration-200 hover:bg-slate-100 hover:text-slate-800'
      >
        {action.icon}
      ...
      <div className='p-6 pb-4'>
        <div className='mb-4 flex items-center justify-between'>
          <div className='flex items-center space-x-3'>
            <div
              className={`h-12 w-12 bg-gradient-to-r ${getCardColor(type)} flex items-center justify-center rounded-xl text-white`}
            >
              {getCardIcon(type)}
            </div>
            <div>
              <h2 className='text-lg font-bold capitalize text-slate-800'>{type}</h2>
            ...
            <div className='space-y-3 text-sm'>
              ...
              <div className='grid grid-cols-2 gap-4'>
                <div>
                  ...
                  <p className='mt-1 text-slate-600'>{formatDate(data.createdAt)}</p>
                </div>
                <div>
                  <span className='font-medium text-slate-700'>Atualizado em:</span>
                  <p className='mt-1 text-slate-600'>{formatDate(data.updatedAt)}</p>
                </div>
              </div>
              ...
              {data.stats && (
                <div className='border-t border-slate-100 pt-3'>
                  <h4 className='mb-2 font-medium text-slate-700'>Estatísticas</h4>
                  <div className='grid grid-cols-2 gap-3'>
                    {Object.entries(data.stats).map(([key, value]) => (
                      <div key={key}>
                        <span className='capitalize text-slate-500'>{key}:</span>
                        <p className='font-medium text-slate-600'>{formatNumber(value)}</p>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {data.metadata && (
                <div className='border-t border-slate-100 pt-3'>
                  <h4 className='mb-2 font-medium text-slate-700'>Metadados</h4>
