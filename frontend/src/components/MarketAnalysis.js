...
const MarketAnalysis = () => {
  ...
  const loadTechnicalIndicators = useCallback(async () => {
    try {
      const historicalData = await cryptoService.getBinanceMarketData(
        selectedCrypto,
        timeframe === '1d' ? 1 : parseInt(timeframe),
        'usd'
      )
  ...
  const calculateTechnicalIndicators = data => {
    ...
    const sma20 = prices.length >= 20 ? prices.slice(-20).reduce((sum, price) => sum + price, 0) / 20 : null

    // Média Móvel Simples (SMA) - 50 períodos
    const sma50 = prices.length >= 50 ? prices.slice(-50).reduce((sum, price) => sum + price, 0) / 50 : null
  ...
  return (
    <div className='mx-auto max-w-7xl space-y-8'>
      ...
      <div className='text-center'>
        <h2 className='mb-4 text-3xl font-bold text-gray-900'>Análise de Mercado</h2>
      ...
      <div className='rounded-lg border border-gray-200 bg-white p-6'>
        <div className='flex flex-col gap-4 sm:flex-row'>
          ...
          <div className='sm:w-auto'>
            <label className='mb-2 block text-sm font-medium text-gray-700'>&nbsp</label>
            <button
              onClick={loadCryptoData}
              className='flex w-full items-center justify-center space-x-2 rounded-lg bg-emerald-600 px-6 py-2 text-white transition-colors hover:bg-emerald-700 sm:w-auto'
            >
      ...
      {cryptoData && (
        <div className='grid grid-cols-1 gap-8 lg:grid-cols-3'>
          ...
          <div className='space-y-6 lg:col-span-2'>
            ...
            <div className='rounded-lg border border-gray-200 bg-white p-6'>
              ...
              <div className='grid grid-cols-2 gap-4 border-t border-gray-200 pt-4 md:grid-cols-4'>
                ...
                <div className='text-center'>
                  ...
                  <div className='font-semibold text-gray-900'>
                    {cryptoData.market_data?.max_supply ? formatNumber(cryptoData.market_data.max_supply) : '∞'}
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className='rounded-lg border border-gray-200 bg-white p-6'>
              <h3 className='mb-4 text-lg font-semibold text-gray-900'>Indicadores Técnicos</h3>
              ...
              <div className='grid grid-cols-1 gap-6 md:grid-cols-2'>
                ...
                <div className='space-y-3'>
                  ...
                  <div className='text-xs text-gray-500'>Status: {getRSIStatus(technicalIndicators.rsi)}</div>
                </div>
                  ...
                  <div className='space-y-2'>
                    <div className='flex justify-between text-sm'>
                      ...
                      <span className='font-mono'>
                        {technicalIndicators.macd?.macdLine ? technicalIndicators.macd.macdLine.toFixed(4) : 'N/A'}
                      </span>
                    </div>
                    <div className='flex justify-between text-sm'>
                      <span>Linha Sinal:</span>
                      <span className='font-mono'>
                        {technicalIndicators.macd?.signalLine ? technicalIndicators.macd.signalLine.toFixed(4) : 'N/A'}
                      </span>
                    </div>
                    <div className='flex justify-between text-sm'>
                      <span>Histograma:</span>
                      <span className='font-mono'>
                        {technicalIndicators.macd?.histogram ? technicalIndicators.macd.histogram.toFixed(4) : 'N/A'}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className='space-y-3'>
                  ...
                  <div className='space-y-2'>
                    <div className='flex justify-between text-sm'>
                      <span>SMA (20):</span>
                      <span className='font-mono'>{formatCurrency(technicalIndicators.sma20)}</span>
                    </div>
                    <div className='flex justify-between text-sm'>
                      <span>SMA (50):</span>
                      <span className='font-mono'>{formatCurrency(technicalIndicators.sma50)}</span>
                    </div>
                    <div className='flex justify-between text-sm'>
                      <span>Preço Atual:</span>
                      <span className='font-mono'>{formatCurrency(technicalIndicators.currentPrice)}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className='space-y-3'>
                  ...
                  <div className='space-y-2'>
                    <div className='flex justify-between text-sm'>
                      ...
                      <span className='font-mono'>{formatCurrency(technicalIndicators.bollingerBands?.upper)}</span>
                    </div>
                    <div className='flex justify-between text-sm'>
                      <span>Média:</span>
                      <span className='font-mono'>{formatCurrency(technicalIndicators.bollingerBands?.middle)}</span>
                    </div>
                    <div className='flex justify-between text-sm'>
                      <span>Banda Inferior:</span>
                      <span className='font-mono'>{formatCurrency(technicalIndicators.bollingerBands?.lower)}</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            {marketData.symbol && (
              <div className='rounded-lg border border-gray-200 bg-white p-6'>
                <h3 className='mb-4 text-lg font-semibold text-gray-900'>Dados de Mercado (Binance)</h3>

                <div className='grid grid-cols-2 gap-4 md:grid-cols-4'>
                  <div className='text-center'>
                    <div className='text-sm text-gray-600'>Volume 24h</div>
                    <div className='font-semibold text-gray-900'>{formatCurrency(marketData.volume)}</div>
                  </div>
                  <div className='text-center'>
                  ...
                  <div className='text-center'>
                    ...
                    <div className='font-semibold text-gray-900'>{formatCurrency(marketData.highPrice)}</div>
                  </div>
                  <div className='text-center'>
                    <div className='text-sm text-gray-600'>Preço Mínimo</div>
                    <div className='font-semibold text-gray-900'>{formatCurrency(marketData.lowPrice)}</div>
                  </div>
                </div>
              </div>
            )}
          ...
          <div className='space-y-6'>
            {/* Notícias do Mercado */}
            <div className='rounded-lg border border-gray-200 bg-white p-6'>
              <h3 className='mb-4 text-lg font-semibold text-gray-900'>Notícias do Mercado</h3>
                ...
                {news.map(item => (
                  <div key={item.id} className='border-l-4 border-gray-200 pl-4'>
                    <div className='flex items-start space-x-2'>
                      ...
                      <div className='flex-1'>
                        <h4 className='mb-1 text-sm font-medium text-gray-900'>{item.title}</h4>
                        <p className='mb-2 text-xs text-gray-600'>{item.summary}</p>
                        <div className='flex items-center justify-between'>
                          <span
                            className={`rounded-full border px-2 py-1 text-xs ${getSentimentColor(item.sentiment)}`}
                          >
            ...
            <div className='rounded-lg border border-gray-200 bg-white p-6'>
              <h3 className='mb-4 text-lg font-semibold text-gray-900'>Sobre {cryptoData.name}</h3>

              <div className='space-y-3'>
                <div>
                  <span className='text-sm text-gray-600'>Categoria:</span>
                  <p className='text-sm font-medium text-gray-900'>{cryptoData.categories?.join(', ') || 'N/A'}</p>
                </div>
