...
import { Route, Loader2, CheckCircle, Clock, AlertCircle, DollarSign, Zap, ExternalLink } from 'lucide-react'
...
const RouterManager = ({ userId }) => {
  ...
  const fetchRouterData = async () => {
    ...
    try {
      ...
      setError(t('router.error', 'Erro ao carregar dados de roteadores'))
    } finally {
      setLoading(false)
    }
  }
    ...
    return (
      <div className="flex items-center justify-center h-64">
        ...
        <span className="ml-3 text-gray-600 dark:text-gray-300">
          {t('router.loading', 'Carregando dados de roteadores...')}
        </span>
      </div>
    )
  }
    ...
    >
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl font-bold text-gray-900 dark:text-white flex items-center">
          <Route className="w-6 h-6 mr-2 text-agro-emerald" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-600 dark:text-gray-400">
                {t('router.activeRouters', 'Roteadores Ativos')}
              </p>
              <p className="text-2xl font-bold text-gray-900 dark:text-white">
                {activeRouters}
              </p>
        ...
        <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-600 dark:text-gray-400">
                {t('router.totalRouters', 'Total de Roteadores')}
              </p>
              <p className="text-2xl font-bold text-gray-900 dark:text-white">
                {routers.length}
              </p>
      ...
      {routers.length === 0 ? (
        <div className="text-center py-8">
          ...
          <p className="text-gray-600 dark:text-gray-400 mb-4">
            {t('router.noRouters', 'Nenhum roteador encontrado')}
          </p>
        </div>
      ) : (
