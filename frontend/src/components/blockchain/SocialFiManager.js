...
const SocialFiManager = ({ userId }) => {
  ...
  const [socialFi, setSocialFi] = useState([])
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState('')
  const [totalValue, setTotalValue] = useState(0)
  const [activeSocialFi, setActiveSocialFi] = useState(0)
  ...
  const fetchSocialFiData = async () => {
    ...
    try {
      ...
      if (data.success) {
        setSocialFi(data.socialFi)
        setTotalValue(data.totalValue)
        setActiveSocialFi(data.activeSocialFi)
      } else {
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-600 dark:text-gray-400">
                {t('socialfi.totalValue', 'Valor Total')}
              </p>
        ...
        <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-600 dark:text-gray-400">
                {t('socialfi.activeSocialFi', 'SocialFi Ativos')}
              </p>
              <p className="text-2xl font-bold text-gray-900 dark:text-white">
                {activeSocialFi}
              </p>
        ...
        <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-600 dark:text-gray-400">
                {t('socialfi.totalSocialFi', 'Total de SocialFi')}
              </p>
              <p className="text-2xl font-bold text-gray-900 dark:text-white">
                {socialFi.length}
              </p>
      ...
      {socialFi.length === 0 ? (
        <div className="text-center py-8">
          ...
          <p className="text-gray-600 dark:text-gray-400 mb-4">
            {t('socialfi.noSocialFi', 'Nenhum SocialFi encontrado')}
          </p>
        </div>
      ) : (
          ...
          {socialFi.map((item) => (
            <div key={item.id} className="border border-gray-200 dark:border-gray-700 rounded-lg p-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm mb-4">
                ...
                <div>
                  <p className="text-gray-600 dark:text-gray-400">
                    {t('socialfi.transactions', 'Transações')}:
                  </p>
                  <p className="font-medium text-gray-900 dark:text-white">
                    {item.transactions}
                  </p>
              ...
              <div className="flex items-center justify-between">
                <div className="text-sm text-gray-500 dark:text-gray-400">
                  {t('socialfi.lastActivity', 'Última Atividade')}: {item.lastActivity}
                </div>
