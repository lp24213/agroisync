...
import { Server, Loader2, CheckCircle, Clock, AlertCircle, DollarSign, Zap, ExternalLink } from 'lucide-react'
...
const NodeManager = ({ userId }) => {
  ...
  const fetchNodeData = async () => {
    ...
    try {
      ...
      setError(t('node.error', 'Erro ao carregar dados de nós'))
    } finally {
      setLoading(false)
    }
  }
    ...
    return (
      <div className="flex items-center justify-center h-64">
        ...
        <span className="ml-3 text-gray-600 dark:text-gray-300">
          {t('node.loading', 'Carregando dados de nós...')}
        </span>
      </div>
    )
  }
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-600 dark:text-gray-400">
                {t('node.totalNodes', 'Total de Nós')}
              </p>
              <p className="text-2xl font-bold text-gray-900 dark:text-white">
                {nodes.length}
              </p>
      ...
      {nodes.length === 0 ? (
        <div className="text-center py-8">
          ...
          <p className="text-gray-600 dark:text-gray-400 mb-4">
            {t('node.noNodes', 'Nenhum nó encontrado')}
          </p>
        </div>
      ) : (
