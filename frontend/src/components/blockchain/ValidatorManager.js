...
const ValidatorManager = ({ userId }) => {
  ...
  const fetchValidatorData = async () => {
    ...
    try {
      ...
      setError(t('validator.error', 'Erro ao carregar dados de validadores'))
    } finally {
      setLoading(false)
    }
  }
    ...
    return (
      <div className="flex items-center justify-center h-64">
        ...
        <span className="ml-3 text-gray-600 dark:text-gray-300">
          {t('validator.loading', 'Carregando dados de validadores...')}
        </span>
      </div>
    )
  }
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-600 dark:text-gray-400">
                {t('validator.activeValidators', 'Validadores Ativos')}
              </p>
              <p className="text-2xl font-bold text-gray-900 dark:text-white">
                {activeValidators}
              </p>
        ...
        <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-600 dark:text-gray-400">
                {t('validator.totalValidators', 'Total de Validadores')}
              </p>
              <p className="text-2xl font-bold text-gray-900 dark:text-white">
                {validators.length}
              </p>
      ...
      {validators.length === 0 ? (
        <div className="text-center py-8">
          ...
          <p className="text-gray-600 dark:text-gray-400 mb-4">
            {t('validator.noValidators', 'Nenhum validador encontrado')}
          </p>
        </div>
      ) : (
