...
const StockTicker = () => {
  ...
  useEffect(() => {
    ...
    const mockStocks = [
      ...
      { symbol: 'SUZB3', name: 'Suzano', price: 52.18, change: 0.67, changePercent: 1.3 },
      { symbol: 'JBSS3', name: 'JBS', price: 28.45, change: -0.23, changePercent: -0.8 },
  ...
  return (
    <div className='stock-ticker'>
      ...
      >
        <div className='flex items-center gap-8 text-sm'>
          <span className='text-primary font-semibold'>B3 - Bolsa de Valores</span>

          {stocks.map((stock, index) => (
            <div key={stock.symbol} className='flex items-center gap-2'>
              <span className='text-primary font-medium'>{stock.symbol}</span>
