...
const TickerB3 = () => {
  ...
  return (
    <div className='h-14 max-w-4xl overflow-hidden rounded-lg border border-white/10 bg-neutral-950/80 backdrop-blur-sm'>
      <div className='flex h-full items-center'>
        <div className='border-r border-white/10 bg-emerald-500/20 px-3 py-1'>
            ...
            {isLoading ? (
              <div className='flex items-center space-x-6 px-4'>
                {[...Array(8)].map((_, i) => (
                  <div key={i} className='flex items-center space-x-2'>
                    ...
                    <div className='h-3 w-8 animate-pulse rounded bg-neutral-800'></div>
                  </div>
                ))}
              </div>
            ) : (
              <div className='flex items-center space-x-6 px-4'>
                {tickerData.map((item, index) => (
                  ...
                  >
                    <span className='text-xs font-semibold text-neutral-100'>{item.symbol}</span>
                    <span className='text-xs text-neutral-300'>R$ {item.price.toFixed(2)}</span>
