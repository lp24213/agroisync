        ...
        <svg className="w-full h-full" viewBox="0 0 400 200">
          ...
          <path
            d={chartData.map((point, index) => {
              ...
              const y = 180 - ((point.price - Math.min(...chartData.map(p => p.price))) / 
                              (Math.max(...chartData.map(p => p.price)) - Math.min(...chartData.map(p => p.price)))) * 150;
