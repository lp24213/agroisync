...
const Contato = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Usar o serviço de contato para enviar mensagem
  ...
  const contactInfo = [
    {
      ...
      content: process.env.REACT_APP_SITE_LOCATION || 'Sinop - MT, Brasil',
              color: 'from-emerald-600 to-blue-600'
    },
    {
      icon: <Phone className="w-6 h-6 text-agro-gold" />,
      title: t('contact.info.phone.title'),
      content: process.env.REACT_APP_SITE_PHONE || '(66) 99236-2830',
              color: 'from-yellow-500 to-blue-600'
    },
    {
      icon: <Mail className="w-6 h-6 text-tech-blue" />,
      title: t('contact.info.email.title'),
      content: process.env.REACT_APP_CONTACT_EMAIL || 'contato@agroisync.com',
