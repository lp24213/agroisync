...
const Onboarding = () => {
  ...
  const [form, setForm] = useState({
    // comum
    address: '', city: '', state: '', ie: '', cpf: '', cnpj: '', zipCode: '',
    // frete
    licensePlate: '', vehicleModel: '', vehicleType: '', capacity: '', vehicleImage: null,
    freightDescription: '', freightPrice: '', freightRoute: '', freightAvailability: '',
    // loja
    productName: '', productDesc: '', productSize: '', productQuantity: '', productImage: null,
    productCategory: '', productPrice: '', productStock: '', productWeight: ''
  });
  ...
  return (
    <section className="container" style={{ padding: '2rem 0', maxWidth: '800px', margin: '0 auto' }}>
      <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
        <h1 className="text-3xl font-bold mb-2" style={{ color: 'var(--text-primary)' }}>Complete seu Perfil</h1>
      ...
      <div className="mb-6" style={{ display: 'flex', gap: '1rem', justifyContent: 'center' }}>
        <label style={{ 
          ...
          <input type="radio" name="t" value="carrier" checked={type==='carrier'} onChange={()=>setType('carrier')} style={{ margin: 0 }} />
          <span style={{ fontWeight: '600', color: type === 'carrier' ? 'var(--accent)' : 'var(--text-primary)' }}>🚛 Frete</span>
        </label>
        <label style={{ 
          display: 'flex', 
          alignItems: 'center', 
          ...
          <input type="radio" name="t" value="store" checked={type==='store'} onChange={()=>setType('store')} style={{ margin: 0 }} />
          <span style={{ fontWeight: '600', color: type === 'store' ? 'var(--accent)' : 'var(--text-primary)' }}>🏪 Loja</span>
        </label>
      </div>

      <form onSubmit={submit} className="grid gap-4">
        {/* Dados Básicos */}
        <div style={{ background: 'var(--card-bg)', padding: '1.5rem', borderRadius: '12px', border: '1px solid rgba(15, 15, 15, 0.05)' }}>
          <h3 style={{ fontSize: '1.2rem', fontWeight: '600', marginBottom: '1rem', color: 'var(--text-primary)' }}>📍 Localização</h3>
        ...
        {/* Documentos */}
        <div style={{ background: 'var(--card-bg)', padding: '1.5rem', borderRadius: '12px', border: '1px solid rgba(15, 15, 15, 0.05)' }}>
          <h3 style={{ fontSize: '1.2rem', fontWeight: '600', marginBottom: '1rem', color: 'var(--text-primary)' }}>📄 Documentos</h3>
        ...
        {type==='carrier' ? (
          <>
            {/* Dados do Veículo */}
            <div style={{ background: 'var(--card-bg)', padding: '1.5rem', borderRadius: '12px', border: '1px solid rgba(15, 15, 15, 0.05)' }}>
              <h3 style={{ fontSize: '1.2rem', fontWeight: '600', marginBottom: '1rem', color: 'var(--text-primary)' }}>🚛 Veículo</h3>
              <div className="grid gap-3">
                <div className="grid grid-cols-3 gap-3">
                  <input 
                    className="form-input" 
                  ...
                  />
                  <input className="form-input" placeholder="Tipo" value={form.vehicleType} onChange={set('vehicleType')} />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <input className="form-input" placeholder="Capacidade (kg)" value={form.capacity} onChange={set('capacity')} />
                  <input type="file" className="form-input" accept="image/*" onChange={handleImageUpload('vehicleImage')} />
                </div>
              </div>
            </div>
            ...
            {/* Dados do Frete */}
            <div style={{ background: 'var(--card-bg)', padding: '1.5rem', borderRadius: '12px', border: '1px solid rgba(15, 15, 15, 0.05)' }}>
              <h3 style={{ fontSize: '1.2rem', fontWeight: '600', marginBottom: '1rem', color: 'var(--text-primary)' }}>💰 Serviços de Frete</h3>
              <div className="grid gap-3">
                <textarea 
                  className="form-input" 
                ...
                />
                <div className="grid grid-cols-3 gap-3">
                  <input className="form-input" placeholder="Preço por km (R$)" value={form.freightPrice} onChange={set('freightPrice')} />
                  <input className="form-input" placeholder="Rota principal" value={form.freightRoute} onChange={set('freightRoute')} />
                  <input className="form-input" placeholder="Disponibilidade" value={form.freightAvailability} onChange={set('freightAvailability')} />
                </div>
              </div>
            </div>
          </>
          ...
          <>
            {/* Dados do Produto */}
            <div style={{ background: 'var(--card-bg)', padding: '1.5rem', borderRadius: '12px', border: '1px solid rgba(15, 15, 15, 0.05)' }}>
              <h3 style={{ fontSize: '1.2rem', fontWeight: '600', marginBottom: '1rem', color: 'var(--text-primary)' }}>📦 Produto</h3>
              <div className="grid gap-3">
                <input 
                  className="form-input" 
                ...
                <div className="grid grid-cols-4 gap-3">
                  ...
                  />
                  <input className="form-input" placeholder="Tamanho" value={form.productSize} onChange={set('productSize')} />
                  <input className="form-input" placeholder="Quantidade" value={form.productQuantity} onChange={set('productQuantity')} />
                  <input type="file" className="form-input" accept="image/*" onChange={handleImageUpload('productImage')} />
                </div>
                <div className="grid grid-cols-3 gap-3">
                  <input 
                    className="form-input" 
                  ...
                  />
                  <input className="form-input" placeholder="Estoque disponível" value={form.productStock} onChange={set('productStock')} />
                  <input className="form-input" placeholder="Peso (kg)" value={form.productWeight} onChange={set('productWeight')} />
                </div>
              </div>
            </div>
          </>
