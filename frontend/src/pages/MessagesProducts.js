...
const MessagesProducts = () => {
  ...
  const checkActivePlan = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setHasActivePlan(userData.subscriptions?.store?.active || false);
      }
    } catch (error) {
  ...
  const loadConversations = async () => {
    try {
      const result = await messagingService.getConversations('product');
      if (result.ok) {
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <div className="max-w-md w-full text-center">
          ...
          <p className="text-gray-600 mb-6">
            Para acessar esta mensageria, finalize o pagamento de sua assinatura.
          </p>
                        ...
                        >
                          ...
                          >
                            ...
                            <p className={`text-xs mt-1 ${
                              message.remetente === user.id ? 'text-blue-100' : 'text-gray-500'
                            }`}>
