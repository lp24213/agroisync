...
export function APRWidget() {
  ...
  const currentPool = stakingPools.find(pool => 
    pool.name.toLowerCase().includes(selectedPool)
  ) || stakingPools[0];

  const getChartData = () => {
    const dataKey = selectedPool === 'flexible' ? 'flexible' : 
                   selectedPool === '30-day' ? 'lock30' :
                   selectedPool === '90-day' ? 'lock90' : 'lock180';
