...
export function TVLWidget() {
  ...
  const getMetricLabel = () => {
    switch (selectedMetric) {
      ...
      case 'pools': return 'Active Pools';
      default: return 'Total Value Locked';
    }
  };

  const getMetricValue = () => {
    switch (selectedMetric) {
      case 'tvl': return formatCurrency(protocolStats.totalValueLocked);
      case 'users': return formatNumber(protocolStats.totalUsers);
      case 'pools': return protocolStats.activePools.toString();
      default: return formatCurrency(protocolStats.totalValueLocked);
    }
  };
        ...
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={getChartData()}>
            ...
            <YAxis 
              stroke="#9ca3af"
              fontSize={10}
              tickFormatter={(value) => selectedMetric === 'tvl' ? `$${(value / 1e6).toFixed(0)}M` : value.toString()}
            />
            <Tooltip 
              contentStyle={{
      ...
      <div className="space-y-3">
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="p-3 bg-white/5 rounded-lg">
            ...
            <p className="text-sm font-medium text-white">
              {protocolStats.securityScore}/10
            </p>
          </div>
        </div>
      ...
      <div className="mt-4 pt-4 border-t border-white/10">
        ...
        <div className="space-y-2">
          {topPools.map((pool, index) => (
            <div key={pool.name} className="flex items-center justify-between p-2 bg-white/5 rounded-lg">
