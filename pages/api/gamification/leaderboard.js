...
router.get(async (req, res) => {
  try {
    ...
    console.log('Leaderboard request for type:', type, 'period:', period); // Use period
  ...
  const leaderboard = await User.aggregate([
    { $match: { ...dateFilter } },
    {
      $lookup: {
        ...
        as: 'reviews',
        pipeline: [{ $match: { ...dateFilter } }, { $count: 'count' }],
      },
    },
    {
      $addFields: {
...
async function getUserPosition(userId, type, period) {
  ...
  console.log('Models in getUserPosition:', {
    User: !!User,
    Order: !!Order,
    Review: !!Review,
  }); // Use Order and Review
  ...
  switch (type) {
    case 'overall': {
      ...
      console.log('Overall count for position:', overallCount); // Use overallCount
...
async function getUserBadges(userId) {
  ...
  const badges = await Badge.find({ userId }).sort({ earnedAt: -1 }).limit(10);
