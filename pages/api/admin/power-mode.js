...
async function getRecentActivity() {
  ...
  const [recentUsers, recentOrders, recentPayments, recentKYC] = await Promise.all([
    User.find().sort({ createdAt: -1 }).limit(5).select('name email role createdAt'),
    Order.find().sort({ createdAt: -1 }).limit(5).select('productType quantity status createdAt'),
    Payment.find().sort({ createdAt: -1 }).limit(5).select('amount status createdAt'),
    KYC.find().sort({ uploadedAt: -1 }).limit(5).select('documentType status uploadedAt')
  ])
...
function getHeadersForDataType(dataType) {
  const headers = {
    ...
    orders: ['productType', 'quantity', 'destination', 'status', 'totalAmount', 'createdAt'],
