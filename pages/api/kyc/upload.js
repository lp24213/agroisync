...
router.post(async (req, res) => {
  try {
    ...
    const kycDocument = new KYC({
      ...
      status: fraudDetection.isFraud ? 'rejected' : 
              validation.isValid ? 'approved' : 'pending_review',
      uploadedAt: new Date()
    })
    ...
    await User.findByIdAndUpdate(decoded.userId, {
      ...
      kycDocuments: [...(await User.findById(decoded.userId)).kycDocuments || [], kycDocument._id]
    })
