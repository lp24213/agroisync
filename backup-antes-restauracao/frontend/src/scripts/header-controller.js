...
class AgroisyncHeaderController {
  ...
  updateHeaderState() {
    ...
    if (!this.header || !this.header.classList) return;
    if (isOverHero) {
      if (this.header && this.header.classList && typeof this.header.classList.add === 'function') {
  ...
  loadHeroImage(imageUrl) {
    ...
    img.onload = () => {
      ...
      if (this.hero && this.hero.classList && typeof this.hero.classList.add === 'function') this.hero.classList.add('loaded');
    };
    img.onerror = () => {
