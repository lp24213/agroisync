...
(function () {
  ...
  function checkHero() {
    ...
    if (hero && hero.getBoundingClientRect().top <= 0) {
      if (header && header.classList && typeof header.classList.add === 'function') header.classList.add('header-over-image');
    } else {
      header.classList.remove('header-over-image');
    }
  }

  window.addEventListener('DOMContentLoaded', checkHero);
