...
class SecurityService {
  constructor() {
    // INICIALIZAÇÃO SEGURA: Garantir que todos os Sets e Maps são criados ANTES de qualquer uso
    ...
    // Verificar novamente após inicialização para garantir que estão válidos
    if (!this.blockedIPs || !(this.blockedIPs instanceof Set)) {
      this.blockedIPs = new Set();
    }
    if (!this.suspiciousIPs || !(this.suspiciousIPs instanceof Set)) {
      this.suspiciousIPs = new Set();
    }
    if (!this.requestLog || !(this.requestLog instanceof Map)) {
      this.requestLog = new Map();
    }
  ...
  blockIP(ip, reason) {
    // GUARD CLAUSE: Garantir que blockedIPs está inicializado antes de usar .add()
    if (!this.blockedIPs || typeof this.blockedIPs.add !== 'function') {
      this.blockedIPs = new Set();
    }
