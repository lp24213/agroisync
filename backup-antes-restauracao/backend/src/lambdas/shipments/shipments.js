  ...
  try {
    ...
    if (event.httpMethod === 'GET') {
      ...
      if (id) {
        ...
        // normalize id for d1 or mongodb
        const normalizedId = d1Client.asObjectId(id);
        // For mongodb ObjectId, validate format
        if (!process.env.USE_D1 === 'true' && id && !ObjectId.isValid(id)) {
          return {
            statusCode: 400,
            headers: corsHeaders,
            body: JSON.stringify({
        ...
        const shipment = await db.collection('shipments').findOne({ _id: normalizedId });
        if (!shipment) {
          return {
            statusCode: 404,
            headers: corsHeaders,
