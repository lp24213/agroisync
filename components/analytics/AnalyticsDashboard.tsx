      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <NeonButton onClick={refreshData} variant="secondary" size="sm" loading={loading}>
              <RefreshCw className="w-4 h-4 mr-2" />
              Refresh
            </NeonButton>

            <NeonButton onClick={exportData} variant="secondary" size="sm">
          ...
          <AnimatedCard key={index} className="p-6">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center">
                {metric.metric === 'Total Value Locked' && (
                  <DollarSign className="w-5 h-5 text-blue-400 mr-2" />
                )}
                {metric.metric === 'Active Users' && (
                  <Users className="w-5 h-5 text-green-400 mr-2" />
                )}
                {metric.metric === 'Transaction Volume' && (
                  <Activity className="w-5 h-5 text-purple-400 mr-2" />
                )}
                {metric.metric === 'Revenue' && (
                  <TrendingUp className="w-5 h-5 text-yellow-400 mr-2" />
                )}
                {metric.metric === 'Carbon Offset' && (
