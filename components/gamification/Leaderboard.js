...
const Leaderboard = () => {
  ...
  useEffect(() => {
    fetchLeaderboard();
  }, [selectedType, selectedPeriod, fetchLeaderboard]);

  const fetchLeaderboard = useCallback(async () => {
    setLoading(true);
    try {
      const response = await fetch(
        `/api/gamification/leaderboard?type=${selectedType}&period=${selectedPeriod}`,
        {
          headers: {
      ...
      <div className="flex items-center justify-center p-8">
        <div className="text-center">
          ...
          <p className="text-gray-600">
            {t('gamification.loading', 'Carregando leaderboard...')}
          </p>
        </div>
      </div>
    );
  ...
  if (!leaderboardData) {
    return (
      <div className="text-center p-8">
        ...
        <p className="text-gray-600">
          {t('gamification.errorLoading', 'Erro ao carregar leaderboard')}
        </p>
      </div>
    );
  }
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">
                {t(
                  'gamification.competitionDescription',
                  'Competição entre usuários da plataforma'
                )}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center space-x-2">
            ...
            >
              <option value="overall">
                {t('gamification.overall', 'Geral')}
              </option>
              <option value="buyers">
                {t('gamification.buyers', 'Compradores')}
              </option>
              <option value="sellers">
                {t('gamification.sellers', 'Vendedores')}
              </option>
              <option value="drivers">
                {t('gamification.drivers', 'Entregadores')}
              </option>
              <option value="reviews">
                {t('gamification.reviews', 'Avaliações')}
              </option>
            </select>
          </div>
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-blue-100">
                  {t('gamification.positionDescription', 'Você está em')} #
                  {leaderboardData.userPosition.position}{' '}
                  {t('gamification.of', 'de')}{' '}
                  {leaderboardData.userPosition.total}
                </p>
              </div>
            </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      ...
                      <div>
                        ...
                        <div className="space-y-1 text-sm text-gray-600">
                          <p>
                            {t('gamification.level', 'Nível')}:{' '}
                            {user.gamification?.level || 1}
                          </p>
                          <p>
                            {t('gamification.score', 'Pontuação')}:{' '}
                            {formatScore(user.gamification?.score || 0)}
                          </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {leaderboardData.userBadges.map((badge, index) => (
              ...
              >
                <div className="flex items-center space-x-3">
                  <div
                    className={`w-10 h-10 bg-${badge.color}-100 rounded-full flex items-center justify-center`}
                  >
