...
export default {
  async findById(id) {
    if (USE_D1) {
      ...
      const db = client.db();
      return db.collection('news').findOne({ id: asId(id) });
    }
  ...
  async findOne(filter = {}) {
    if (USE_D1) {
      const client = await d1Client.connect();
      const db = client.db();
      return db.collection('news').findOne(filter);
    }
  ...
  async find(filter = {}, opts = {}) {
    if (USE_D1) {
      const client = await d1Client.connect();
      const db = client.db();
      const cursor = db.collection('news').find(filter);
  ...
  async count(filter = {}) {
    if (USE_D1) {
      const client = await d1Client.connect();
      const db = client.db();
      return db.collection('news').countDocuments(filter);
    }
  ...
  async create(data) {
    if (USE_D1) {
      const client = await d1Client.connect();
      const db = client.db();
      const res = await db.collection('news').insertOne(data);
      return res;
    }
  ...
  async update(filter, update, opts = {}) {
    if (USE_D1) {
      const client = await d1Client.connect();
      const db = client.db();
      return db.collection('news').updateOne(filter, update, opts);
    }
  ...
  async delete(filter) {
    if (USE_D1) {
      const client = await d1Client.connect();
      const db = client.db();
      return db.collection('news').deleteOne(filter);
    }
