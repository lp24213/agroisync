...
router.get('/', async (req, res) => {
  try {
    ...
    // Execute query (use D1 DAO when enabled)
    let products;
    let total;
    if (USE_D1) {
      products = await ProductDao.find(query, { sort, skip, limit: parseInt(limit, 10, 10) });
      total = await ProductDao.count(query);

      // minimal populate seller info
      const sellerIds = [...new Set(products.map(p => String(p.sellerId)).filter(Boolean))];
      const sellers = {};
      for (const sid of sellerIds) {
        try {
          const s = await UserDao.findById(sid);
          sellers[sid] = s;
        } catch (e) {
          sellers[sid] = null;
        }
      ...
      products = products.map(p => ({
        ...p,
        seller: sellers[String(p.sellerId)]
          ? { name: sellers[String(p.sellerId)].name, company: sellers[String(p.sellerId)].company }
          : undefined
      }));
    } else {
...
router.get('/featured', async (req, res) => {
  try {
    ...
    if (USE_D1) {
      ...
      const sellerIds = [...new Set(products.map(p => String(p.sellerId)).filter(Boolean))];
      const sellers = {};
      for (const sid of sellerIds) sellers[sid] = await UserDao.findById(sid).catch(() => null);
      products = products.map(p => ({
        ...p,
        seller: sellers[String(p.sellerId)]
          ? { name: sellers[String(p.sellerId)].name, company: sellers[String(p.sellerId)].company }
