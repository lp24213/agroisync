...
Este documento explica como configurar a autenticação via ID da Metamask no
backend do AGROTM.
...
O middleware `validateMetamaskId` foi implementado em
`src/middleware/metamaskAuth.js` e:
...
Se a validação for bem-sucedida, a requisição continua normalmente. Caso
contrário, retorna erro 401 ou 403.
...
1. **Validação obrigatória**: Todas as rotas protegidas requerem a ID da
   Metamask
