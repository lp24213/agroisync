...
export const mintNFT = functions.https.onCall(async (data: { to: string; tokenURI: string }) => {
  ...
  if (!nftContract.mint) throw new functions.https.HttpsError('internal', 'Método mint não disponível no contrato');
...
// Transfer NFT
export const transferNFT = functions.https.onCall(async (data: { from: string; to: string; tokenId: string }) => {
  ...
  if (!nftContract.transferFrom) throw new functions.https.HttpsError('internal', 'Método transferFrom não disponível no contrato');
...
export const getNFT = functions.https.onCall(async (data: { tokenId: string }) => {
  ...
  if (!nftContract.ownerOf || !nftContract.tokenURI) throw new functions.https.HttpsError('internal', 'Métodos ownerOf ou tokenURI não disponíveis no contrato');
