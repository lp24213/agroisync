...
import Model from '../models/UserD1.js';
...
export default {
  async findById(id) {
    if (USE_D1) {
      ...
      const db = client.db();
      return db.collection('userd1').findOne({ id: asId(id) });
    }
  ...
  async findOne(filter = {}) {
    if (USE_D1) {
      const client = await d1Client.connect();
      const db = client.db();
      return db.collection('userd1').findOne(filter);
    }
  ...
  async find(filter = {}, opts = {}) {
    if (USE_D1) {
      const client = await d1Client.connect();
      const db = client.db();
      const cursor = db.collection('userd1').find(filter);
  ...
  async count(filter = {}) {
    if (USE_D1) {
      const client = await d1Client.connect();
      const db = client.db();
      return db.collection('userd1').countDocuments(filter);
    }
  ...
  async create(data) {
    if (USE_D1) {
      const client = await d1Client.connect();
      const db = client.db();
      const res = await db.collection('userd1').insertOne(data);
      return res;
    }
  ...
  async update(filter, update, opts = {}) {
    if (USE_D1) {
      const client = await d1Client.connect();
      const db = client.db();
      return db.collection('userd1').updateOne(filter, update, opts);
    }
  ...
  async delete(filter) {
    if (USE_D1) {
      const client = await d1Client.connect();
      const db = client.db();
      return db.collection('userd1').deleteOne(filter);
    }
