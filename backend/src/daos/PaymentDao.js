...
export default {
  async findById(id) {
    if (USE_D1) {
      ...
      const db = client.db();
      return db.collection('payment').findOne({ id: asId(id) });
    }
  ...
  async findOne(filter = {}) {
    if (USE_D1) {
      const client = await d1Client.connect();
      const db = client.db();
      return db.collection('payment').findOne(filter);
    }
  ...
  async find(filter = {}, opts = {}) {
    if (USE_D1) {
      const client = await d1Client.connect();
      const db = client.db();
      const cursor = db.collection('payment').find(filter);
  ...
  async count(filter = {}) {
    if (USE_D1) {
      const client = await d1Client.connect();
      const db = client.db();
      return db.collection('payment').countDocuments(filter);
    }
  ...
  async create(data) {
    if (USE_D1) {
      const client = await d1Client.connect();
      const db = client.db();
      const res = await db.collection('payment').insertOne(data);
      return res;
    }
  ...
  async update(filter, update, opts = {}) {
    if (USE_D1) {
      const client = await d1Client.connect();
      const db = client.db();
      return db.collection('payment').updateOne(filter, update, opts);
    }
  ...
  async delete(filter) {
    if (USE_D1) {
      const client = await d1Client.connect();
      const db = client.db();
      return db.collection('payment').deleteOne(filter);
    }
