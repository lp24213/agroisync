...
import Model from '../models/Chat.js';
...
export default {
  async findById(id) {
    if (USE_D1) {
      ...
      const db = client.db();
      return db.collection('chat').findOne({ id: asId(id) });
    }
  ...
  async findOne(filter = {}) {
    if (USE_D1) {
      const client = await d1Client.connect();
      const db = client.db();
      return db.collection('chat').findOne(filter);
    }
  ...
  async find(filter = {}, opts = {}) {
    if (USE_D1) {
      const client = await d1Client.connect();
      const db = client.db();
      const cursor = db.collection('chat').find(filter);
  ...
  async count(filter = {}) {
    if (USE_D1) {
      const client = await d1Client.connect();
      const db = client.db();
      return db.collection('chat').countDocuments(filter);
    }
  ...
  async create(data) {
    if (USE_D1) {
      const client = await d1Client.connect();
      const db = client.db();
      const res = await db.collection('chat').insertOne(data);
      return res;
    }
  ...
  async update(filter, update, opts = {}) {
    if (USE_D1) {
      const client = await d1Client.connect();
      const db = client.db();
      return db.collection('chat').updateOne(filter, update, opts);
    }
  ...
  async delete(filter) {
    if (USE_D1) {
      const client = await d1Client.connect();
      const db = client.db();
      return db.collection('chat').deleteOne(filter);
    }
