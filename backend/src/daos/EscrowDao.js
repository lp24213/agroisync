...
import Model from '../models/Escrow.js';
...
export default {
  async findById(id) {
    if (USE_D1) {
      ...
      const db = client.db();
      return db.collection('escrow').findOne({ id: asId(id) });
    }
  ...
  async findOne(filter = {}) {
    if (USE_D1) {
      const client = await d1Client.connect();
      const db = client.db();
      return db.collection('escrow').findOne(filter);
    }
  ...
  async find(filter = {}, opts = {}) {
    if (USE_D1) {
      const client = await d1Client.connect();
      const db = client.db();
      const cursor = db.collection('escrow').find(filter);
  ...
  async count(filter = {}) {
    if (USE_D1) {
      const client = await d1Client.connect();
      const db = client.db();
      return db.collection('escrow').countDocuments(filter);
    }
  ...
  async create(data) {
    if (USE_D1) {
      const client = await d1Client.connect();
      const db = client.db();
      const res = await db.collection('escrow').insertOne(data);
      return res;
    }
  ...
  async update(filter, update, opts = {}) {
    if (USE_D1) {
      const client = await d1Client.connect();
      const db = client.db();
      return db.collection('escrow').updateOne(filter, update, opts);
    }
  ...
  async delete(filter) {
    if (USE_D1) {
      const client = await d1Client.connect();
      const db = client.db();
      return db.collection('escrow').deleteOne(filter);
    }
