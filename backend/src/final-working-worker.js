  ...
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === '/api/whatsapp/send-code' && request.method === 'POST') {
      try {
        ...
        console.log(`🚀 ENVIANDO WHATSAPP via TWILIO para ${formattedPhone} com código ${verificationCode}`);
        ...
        if (twilioResponse.ok && twilioData.sid) {
          console.log(`📱 WHATSAPP ENTREGUE via Twilio para ${formattedPhone}: ${verificationCode}`);
