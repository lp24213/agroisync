...
const validateMetamaskId = (req, res, next) => {
  try {
    ...
    if (metamaskId !== authorizedMetamaskId) {
      logger.warn(`Tentativa de acesso com Metamask ID inválido: ${metamaskId} - IP: ${req.ip} - URL: ${req.originalUrl}`);
    ...
    // ID válida, continuar
    logger.info(`Requisição autenticada com Metamask ID: ${metamaskId} - ${req.method} ${req.originalUrl}`);
...
const logMetamaskAccess = (req, res, next) => {
  ...
  logger.info(`Tentativa de acesso - Metamask ID: ${metamaskId || 'N/A'} - IP: ${ip} - User-Agent: ${userAgent} - Timestamp: ${timestamp}`);
