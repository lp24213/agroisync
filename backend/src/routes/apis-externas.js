...
router.post('/validar-cpf', apiLimiter, [
...
router.post('/validar-cnpj', apiLimiter, [
...
router.post('/consultar-cep', apiLimiter, [
...
router.post('/consultar-ibge', apiLimiter, [
...
router.post('/consultar-baidu', apiLimiter, [
...
router.post('/consultar-google', apiLimiter, [
  body('query').isLength({ min: 1, max: 100 }).withMessage('Query inválida')
], async (req, res) => {
  try {
    const errors = validationResult(req);
