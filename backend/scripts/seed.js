...
async function createUsers() {
  ...
  for (let i = 0; i < SEED_COUNT.users - sampleUsers.length; i++) {
    const user = new User({
      ...
      planActive: Math.random() > 0.5 ? ['loja-basic', 'loja-pro', 'agroconecta-basic', 'agroconecta-pro'][i % 4] : null,
      planType: Math.random() > 0.5 ? ['basic', 'pro'][i % 2] : null,
      planExpiry: Math.random() > 0.5 ? new Date(Date.now() + (Math.random() * 365 * 24 * 60 * 60 * 1000)) : null
    });
    users.push(await user.save());
  }
...
async function createPayments(users) {
  ...
  for (const user of paidUsers) {
    const payment = new Payment({
      ...
      amount: user.planActive?.includes('pro') ? 99.90 : 49.90,
      currency: 'BRL',
      ...
      crypto: user.planActive?.includes('pro') ? undefined : {
