...
exports.transferNFT = functions.https.onCall(async (data) => {
    ...
    if (!nftContract.transferFrom)
        throw new functions.https.HttpsError('internal', 'Método transferFrom não disponível no contrato');
...
exports.getNFT = functions.https.onCall(async (data) => {
    ...
    if (!nftContract.ownerOf || !nftContract.tokenURI)
        throw new functions.https.HttpsError('internal', 'Métodos ownerOf ou tokenURI não disponíveis no contrato');
