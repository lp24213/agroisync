...
Este documento fornece instruções detalhadas para fazer o deploy do backend AgroSync no Cloudflare Workers com banco de dados D1.
...
**Importante:** Copie o `database_id` gerado e atualize em `wrangler-worker.toml`:
...
**Solução:** Verifique se `wrangler-worker.toml` tem a configuração correta do D1:
...
**Solução:** Remova rotas duplicadas no dashboard do Cloudflare ou no wrangler.toml
